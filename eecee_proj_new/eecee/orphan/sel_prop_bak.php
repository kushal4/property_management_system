...
if( $count_sql_check_prop > 0){
    ...
    $user_prop_table = new sense_table([
      ...
      $user_prop_table->setParent($user_prop_table_container);
        ...
        $cell_obj = $user_prop_table->addCell("Property Names", TRUE);
        $user_prop_table->setCurrCellID("cell_1");

        
        $cell_obj = $user_prop_table->addCell("Flat Name", TRUE);
        $user_prop_table->setCurrCellID("cell_2");

        $cell_obj = $user_prop_table->addCell("Role", TRUE);
        $user_prop_table->setCurrCellID("cell_3");

        $cell_obj = $user_prop_table->addCell("Details", TRUE);
        $user_prop_table->setCurrCellID("cell_4");

        $cell_obj = $user_prop_table->addCell(" ", TRUE);
        $user_prop_table->setCurrCellID("cell_5");
    ...
    foreach ($sql_check_prop_row as $value){  
       ...
       if($unit_id != 0){
        $cell_obj = $user_prop_table->addCell($unit_name);
        $user_prop_table->setCurrCellID("cell".$pri_id."_2");
       }else{
        ...
        if($unit_id != 0){
            if($user_type == 1){
                $u_type = "owner";
                $cell_obj = $user_prop_table->addCell($u_type);
                $user_prop_table->setCurrCellID("cell".$pri_id."_3");
        } else if($user_type == 2){
                $u_type = "co-owner";
                $cell_obj = $user_prop_table->addCell($u_type);
                $user_prop_table->setCurrCellID("cell".$pri_id."_3");
        }else if($user_type == 3){
                $u_type = "family member";
                $cell_obj = $user_prop_table->addCell($u_type);
                $user_prop_table->setCurrCellID("cell".$pri_id."_3");
            }else if($user_type == 4){
                $u_type = "tenant";
        ...
        $cell_obj = $user_prop_table->addCell("");
        $user_prop_table->setCurrCellID("cell".$pri_id."_4");
        ...
        if($unit_id != 0){
            ...
            addAttribToElement($cell_obj, '{"class":"prp_name_style"}');
            $cell_obj_inside_div = insertElement($cell_obj,"div",'{"class":"enter_wid_ths_role_style"}',"Enter with this role ");
            addAttribToElement($cell_obj, '{"data-prop_id": '.$prop_id.' }');
        }else{
            $cell_obj = $user_prop_table->addCell("");
            $user_prop_table->setCurrCellID("cell".$pri_id."_5");
