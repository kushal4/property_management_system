...
if (is_ajax()) {
    ...
    $feature_id_decoded = sec_get_map_val ("feature_id_map", $feat_val);
    $logfile->logfile_writeline("the encoded feature ID is".$feat_val);
    $logfile->logfile_writeline("the decoded feature ID is".$feature_id_decoded);
    ...
    $sql_insrt="INSERT INTO unit_type_fea(prop_id, unit_type_id, unit_fea_id) VALUES (?, ?, ?)";
        $sql_temp = $conn->prepare($sql_insrt);
        if($sql_temp){
            //echo "prepare successful"."\n";
            $logfile->logfile_writeline("prepare successful");
            $bind_temp = $sql_temp->bind_param("iii", $prop_id, $unit_type_id, $feature_id_decoded);
            if($bind_temp){
