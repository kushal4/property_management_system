...
function createRoleCat(parent_id, node_name, node_type) {
	...
	$.ajax({
		method: "POST",
		url: "create_role_n_category.php",
		data: { k: JSON.stringify(object) },
		dataType: "json",
		success: function (data, textStatus, jQxhr) {
			console.log("add_new_topics::AJAX Return: ");
...
function set_target_role_sig_sess(target_sig, type) {
	...
	$.ajax({
		method: "POST",
		url: "set_target_role_sig_sess.php",
		data: { k: JSON.stringify(object) },
		dataType: "json",
		success: function (data, textStatus, jQxhr) {
			console.log("add_new_topics::AJAX Return: ");
			...
			if (data.ret_code == 4) {
				...
				if (data.ret_code == 0) {
					if (type == "view") {
						...
						var object = {};
                        object.op = "view";
                        $("#permission_cont").load("perm_view.php #main_cont", object, function(data){
							console.log(data);
						});
					} else {
						//window.location.href = 'role_man_upd_perm.php'; //go to role_man_upd_perm
						var object = {};
                        object.op = "upd";
                        $("#permission_cont").load("perm_view.php #main_cont", object, function(data){
							console.log(data);
	...
	$.ajax({
		...
		success: function (data, textStatus, jQxhr) {
			...
			if (data.ret_code == 4) {
				...
				if (data.ret_code == 0) {
					$('.role_tree_man').jstree(true).refresh();
					$(".upd_role_n_cat_dialog_style").dialog('close');
				} else {
...
function update_role_n_category_dialog(obj) {
	...
	var type = obj.type;
	$(".upd_role_n_cat_dialog_style").data("node_id", node_id);
	$(".upd_role_n_cat_dialog_style").data("old_name", old_name);
	$(".upd_role_n_cat_dialog_style").data("type", type);
	$(".upd_role_n_cat_dialog_style").dialog('open');
}
...
function role_tree_manip() {
	...
	$('.role_tree_man').on('rename_node.jstree', function (e, data) {
		...
		$(".upd_role_n_cat_dialog_style").data("node_name", node_name);
		$(".upd_role_n_cat_dialog_style").data("node_id", node_id);
...
$(document).ready(function () {
    ...
    console.log("test");
	$(".upd_role_n_cat_dialog_style").dialog({
		width: 350,
		height: 250,
		dialogClass: 'generic_dialog cant_del_role_cat_dialog_style',
		autoOpen: false,
	...
	$(".upd_role_n_cat_dialog_style").on("dialogopen", function (event, ui) {
		var old_name = $(".upd_role_n_cat_dialog_style").data("old_name");
		$("#name_textbox").val(old_name);
		$("#description_textbox").val("");
	});


	$(".upd_role_n_cat_dialog_style").on("dialogclose", function (event, ui) {
		$('.role_tree_man').jstree(true).refresh();
	});
	...
	$("#update_cont").on('click', function (e) {
		var node_id = $(".upd_role_n_cat_dialog_style").data("node_id");
		var updated_name = $('#name_textbox').val();
		var updated_description = $('#description_textbox').val();
		var type = $(".upd_role_n_cat_dialog_style").data("type");
