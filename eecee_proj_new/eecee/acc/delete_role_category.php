...
if (is_ajax()) {
    ...
    */
    ...
    $data=array("param"=>array("method"=>$deleteRoleCat, "key"=>$projectID, "client_id"=>$prop_id, "client_role"=>$role_sig, "logged_in_role_sig"=>$role_sig, "role_cat_sig"=>$role_cat_sig));
    //print_r($data);
    ...
    $role_arr = explode(",",$message);
    
   

    if($status_value == 0){
        //echo "entered status 0";
        $raw_json["ret_code"] = 0;
        $raw_json_encode=json_encode($raw_json);
        echo $raw_json_encode;
    }else{
        //echo "entered status 1";
        $raw_json["ret_code"] = 1;
